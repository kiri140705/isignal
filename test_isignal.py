...
def run_tests():
    ...
    # 2. 학생 등록
    print("\n[2] 학생 등록 테스트")
    res = RegisterStudent("test_stu_001", "parent_001", "김민준", 12, '["게임", "우주"]')
    print(res)
    ...
    for title, msg in scenarios:
        print(f"\n--- 시나리오: {title} ---")
        print(f"User(아이): {msg}")
        response = json.loads(ChatWithAI("test_stu_001", msg))
    ...
    # 4. 리포트 생성
    print("\n[4] 월간 리포트 생성 테스트")
    report_res = json.loads(GetMonthlyReport("test_stu_001", "2026-07"))
